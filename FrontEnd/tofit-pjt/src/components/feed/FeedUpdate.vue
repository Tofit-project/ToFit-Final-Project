<template>
  <div class="feed-create-container">
    <div class="feed-create-box">
      ...
      <button class="submit-button" @click="regist">완료</button>
    </div>
  </div>
</template>
...
onMounted(() => {
  ...
  feed.value.feedId = route.params.feedId;
  feedStore.getFeedDetail(feed.value.feedId); // 이건 확인

  if (feedStore.feedDetailInfo.value) {
    feed.value.content = feedStore.feedDetailInfo.value.feed.content;
  }
});
...
const regist = function () {
  if (feed.value.content) {
    feedStore.feedUpdate(feed.value);
  } else {
    alert("내용을 작성해주세요.");
  }
};
